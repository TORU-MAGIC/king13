...
function statDelta(u){
  ...
  if(!u)return d;
  if(typeof awakenBonus==='function'){
    d.atk+=awakenBonus(u,'atk');
    d.pdef+=awakenBonus(u,'pdef');
    d.mdef+=awakenBonus(u,'mdef');
    d.mov+=awakenBonus(u,'mov');
  }
  if(u.status){
...
function applyVsIntro(){
  ...
  var arena=document.querySelector('.b-arena');
  if(arena){arena.classList.remove('do-camera-push');void arena.offsetWidth;arena.classList.add('do-camera-push');}
}
...
function showBattleCutin(side,name,sub,crit){
  // ★ユーザー要望: 戦闘中に中央へ横からスライドインしてくる
  //   キャラクター名カットインは不要 → 関数を no-op 化。
  //   既存呼び出し（battle.js）は維持し、ここだけ無効化することで他の演出には影響しない。
  // 念のため残骸が残っていれば消しておく
  var old=document.getElementById('bCutin');
  if(old&&old.parentElement)old.parentElement.removeChild(old);
}
function applyCameraClash(){
  var arena=document.querySelector('.b-arena');
  if(!arena)return;
  arena.classList.remove('do-camera-clash');void arena.offsetWidth;arena.classList.add('do-camera-clash');
  setTimeout(function(){arena.classList.remove('do-camera-clash');},700);
}
function attackStyleLabel(type){
  var ud=UDEFS[type]||{};
  if(type==='dragon')return 'BREATH IMPACT';
  if(type==='catapult')return 'SIEGE STRIKE';
  if(type==='assassin'||type==='ninja')return 'SHADOW EXECUTION';
  if(type==='king')return 'ROYAL COMMAND';
  if(ud.atkType==='magic')return 'ARCANE CAST';
  if(['archer','necromancer','arcanelord','phoenix','healer','monk'].indexOf(type)>=0)return 'RANGED ASSAULT';
