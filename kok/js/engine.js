...
function effAtk(u){
  var b=UDEFS[u.type].atk+awakenBonus(u,'atk')+(GS?GS.weather.atk||0:0)+Math.round(((GS?GS.players[u.owner].morale:100)-100)*0.05)+(u.level-1);
...
function effMov(u){return Math.max(1,UDEFS[u.type].mov+awakenBonus(u,'mov')+(GS?GS.weather.mov||0:0)+fxSum(u,'mov'));}
function mdist(r1,c1,r2,c2){return Math.abs(r1-r2)+Math.abs(c1-c2);}
function critRate(u){return Math.min(.5,((u.type==='assassin'||u.type==='monk')?0.20:u.type==='hero'?0.15:0.10)+awakenBonus(u,'crit'));}
...
function getAttackable(gs,unit){
  var rng=UDEFS[unit.type].rng+awakenBonus(unit,'rng'),res=[];
...
function doKingAoEAction(gs,uid){
  ...
  var targets=[],results=[];
  var aoeRange=KING_AOE_RANGE+(isAwakened(u)?1:0);
  for(var dr=-aoeRange;dr<=aoeRange;dr++)for(var dc=-aoeRange;dc<=aoeRange;dc++){
    if(Math.abs(dr)+Math.abs(dc)>aoeRange)continue;
...
function maybeAwaken(gs,u,oldLv){
  if(!u||!getAwakenDef(u.type)||u.awakened||(u.level||1)<AWAKEN_LEVEL||oldLv>=AWAKEN_LEVEL)return false;
  u.awakened=true;
  var a=getAwakenDef(u.type);
  addLog('✨ '+PCOLS[u.owner].name+'の'+UDEFS[u.type].name+' が【'+a.name+'】に覚醒！ '+(a.desc||''),{hot:true});
  if(typeof showMsg==='function')showMsg('✨ '+UDEFS[u.type].name+' → '+a.name+' 覚醒！',2500);
  return true;
}
function gainXP(gs,u,amt){
  if(!u||u.hp<=0)return;if((u.level||1)>=LEVEL_CAP)return;
  var oldLv=u.level||1;
  u.xp=(u.xp||0)+amt;
  if(u.xp>=100*(u.level||1)){
    u.xp=0;u.level=(u.level||1)+1;u.mhp+=5;u.hp=Math.min(u.hp+5,u.mhp);
    addLog(PCOLS[u.owner].name+'の'+unitName(u.type,u)+'がLv'+u.level+'に！',{sys:true});
    maybeAwaken(gs,u,oldLv);
    SFX.levelup();
  }
}
...
function killLevelUp(gs,killer,killed){
  ...
  var msg=PCOLS[killer.owner].name+'の'+unitName(killer.type,killer)+' Lv'+oldLv+'→Lv'+killer.level+capNote+'！';
  if(actualGained>1)msg+=' (★+'+actualGained+'レベル連続上昇！)';
  addLog(msg,{hot:true});
  var awakened=maybeAwaken(gs,killer,oldLv);
  SFX.levelup();
  // ★マスター到達時の特別演出
  if(reachedMaster){
...
function effPDef(u){return Math.max(0,(UDEFS[u.type].pdef||0)+awakenBonus(u,'pdef')+(u.level-1)+fxSum(u,'pdef'));}
function effMDef(u){return Math.max(0,(UDEFS[u.type].mdef||0)+awakenBonus(u,'mdef')+(u.level-1)+fxSum(u,'mdef'));}
...
function calcAttack(gs,atkId,defId){
  ...
  var res={dmg:totalDmg,cdmg:cdmg,dkill:dkill,ckill:ckill,phxRev:phxRev,affMult:h1.affMult,elemMult:h1.elemMult,isCrit:h1.isCrit,isCritC:isCritC,isDual:atk.type==='dualblader'&&dmg2>0,atkOwner:atk.owner,defOwner:def.owner,atkType:atk.type,defType:def.type,atkAwakened:isAwakened(atk),defAwakened:isAwakened(def),atkHpBef:Math.max(0,atk.hp+cdmg),atkHpAft:Math.max(0,atk.hp),defHpBef:Math.max(0,def.hp+totalDmg),defHpAft:Math.max(0,def.hp),atkMhp:atk.mhp,defMhp:def.mhp,atkStatus:atk.status.slice(),defStatus:def.status.slice(),atkElem:ud.elem,defElem:dd.elem,atkAtkType:ud.atkType,defEnemyLevel:defLevel,tid:MAP[def.row][def.col],
    // ★Bug Fix: battle.js が MASTER 判定で参照する atkId/defId/atkLevel/defLevel を含める
    atkId:atk.id,defId:def.id,atkLevel:atkLevel,defLevel:defLevel,
...
function drawPauseField(){
  ...
  GS.units.forEach(function(u){
    ...
    c.fillText(u.type==='king'?'👑':(typeof unitSym==='function'?unitSym(u.type,u):UDEFS[u.type].sym),cx,cy);
