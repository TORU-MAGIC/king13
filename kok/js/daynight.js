...
/* ===== 夜戦特効テーブル =====
 * 夜間（getCurrentTimePhase()==='night'）の攻撃時、対象ユニットのATKに加算する。
 * 主に dark / swift 系の隠密ユニットを強化、聖系（holy）は逆に弱体化。
 */
var NIGHT_ATK_BONUS={
  ninja:6,        // 🌑 闇に紛れて奇襲
  assassin:8,     // 🌑 暗殺者の真骨頂
  ...
  dragon:3,       // 🔥 ドラゴンの夜行性
  phoenix:-2,     // 🔥 太陽の鳥は夜に弱る
  paladin:-2,     // ✨ 聖騎士は夜に力を失う
  valkyrie:-2,    // ✨ 同上
  hero:-1         // ✨ 英雄もわずかに弱体
};
function getNightAtkBonus(type){
  if(!dayNightEnabled)return 0;
  if(getCurrentTimePhase()!=='night')return 0;
  return NIGHT_ATK_BONUS[type]||0;
}
...
/* ===== 夜戦特効: calcAttack ラッパ =====
 * UDEFS[type].atk を一時的に水増ししてオリジナル calcAttack を呼ぶ。
 * v10.js の calcAttack ラッパ（チェーン/シナジー）と同じ手法。
 * 既に v10.js / scenario.js が calcAttack をラップ済みなので、後付けで重ね掛けする。
 */
(function installNightAtkHook(){
  if(typeof calcAttack!=='function')return;
  if(calcAttack._nightHooked)return;
  var _prevCalcAttack=calcAttack;
  calcAttack=function(gs,atkId,defId){
    var atk=null;
    if(gs&&gs.units){
      for(var i=0;i<gs.units.length;i++){if(gs.units[i].id===atkId){atk=gs.units[i];break;}}
    }
    var nb=atk?getNightAtkBonus(atk.type):0;
    if(nb===0)return _prevCalcAttack(gs,atkId,defId);
    var origAtk=UDEFS[atk.type].atk;
    var res;
    try{
      UDEFS[atk.type].atk=Math.max(1,origAtk+nb);
      res=_prevCalcAttack(gs,atkId,defId);
    }finally{
      UDEFS[atk.type].atk=origAtk;
    }
    // ★リアル GS への適用時のみログ表示（シミュレーション/評価関数では出さない）
    if(res&&gs===GS&&typeof addLog==='function'){
      var sym=nb>0?'🌙夜戦特効':'☀夜の弱体';
      addLog(sym+': '+UDEFS[atk.type].name+' ATK'+(nb>0?'+':'')+nb,{sys:true});
    }
    if(res)res._nightAtkBonus=nb;
    return res;
  };
  calcAttack._nightHooked=true;
})();

/* ===== 起動時: 既に v10.js などで calcAttack がラップされている前提でフック =====
 * DOMContentLoaded でも一応再フック（多重防止は _nightHooked フラグ）
 */
if(typeof window!=='undefined'){
  window.addEventListener('DOMContentLoaded',function(){
    if(typeof calcAttack==='function'&&!calcAttack._nightHooked){
      // 別タイミングで上書きされた場合はもう一度
      (function reinstall(){
        var _prev=calcAttack;
        calcAttack=function(gs,atkId,defId){
          var atk=null;
          if(gs&&gs.units){
            for(var i=0;i<gs.units.length;i++){if(gs.units[i].id===atkId){atk=gs.units[i];break;}}
          ...
          var nb=atk?getNightAtkBonus(atk.type):0;
          if(nb===0)return _prev(gs,atkId,defId);
          var origAtk=UDEFS[atk.type].atk;
          var res;
          try{UDEFS[atk.type].atk=Math.max(1,origAtk+nb);res=_prev(gs,atkId,defId);}
          finally{UDEFS[atk.type].atk=origAtk;}
          if(res&&gs===GS&&typeof addLog==='function'){
            addLog((nb>0?'🌙夜戦特効: ':'☀夜の弱体: ')+UDEFS[atk.type].name+' ATK'+(nb>0?'+':'')+nb,{sys:true});
          }
          if(res)res._nightAtkBonus=nb;
          return res;
        };
