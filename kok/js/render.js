...
function spawnAttackSignature(x1,y1,x2,y2,type,elem,col,isCrit){
  var ud=UDEFS[type]||{},ei=ELEM_INFO[elem||ud.elem||'none']||ELEM_INFO.none;
  col=col||ei.col||'#fff';
  var ranged=['archer','catapult','necromancer','mage','witch','arcanelord','phoenix','healer','monk'];
  if(type==='dragon'){
    for(var i=0;i<36;i++){
      var t=i/35,px=x1+(x2-x1)*t,py=y1+(y2-y1)*t+Math.sin(t*Math.PI)*-22;
      particles.push({x:px,y:py,vx:(Math.random()-.5)*7,vy:(Math.random()-.5)*5-1,sz:Math.random()*18+9,col:i%4?'#ff6b24':'#fff0a0',alpha:1,type:'magic',decay:.024});
    }
    spawnShockwave(x2,y2,'#ff5a24',isCrit?135:100);return;
  }
  if(type==='catapult'){
    for(var j=0;j<18;j++){
      var tt=j/17,px2=x1+(x2-x1)*tt,py2=y1+(y2-y1)*tt+Math.sin(tt*Math.PI)*-95;
      particles.push({x:px2,y:py2,vx:0,vy:0,sz:Math.random()*10+8,col:'#b7a07a',alpha:1,type:'hit',decay:.05});
    }
    spawnShockwave(x2,y2,'#d0b27a',isCrit?120:86);return;
  }
  if(ud.atkType==='magic'||['arcanelord','mage','witch','necromancer','phoenix','healer'].indexOf(type)>=0){
    for(var r=0;r<3;r++)particles.push({x:x2,y:y2,vx:0,vy:0,sz:8+r*9,growRate:5+r*2,maxSz:(isCrit?110:78)+r*18,col:ei.col,alpha:.74-r*.12,type:'ring',decay:.035+r*.007});
    for(var k=0;k<42;k++){
      var a=k/42*Math.PI*2,rad=(k%5)*8;
      particles.push({x:x2+Math.cos(a)*rad,y:y2+Math.sin(a)*rad,vx:Math.cos(a)*(Math.random()*6+2),vy:Math.sin(a)*(Math.random()*6+2)-2,sz:Math.random()*11+4,col:k%3?'#fff':ei.col,alpha:1,type:'magic',decay:.018});
    }
    return;
  }
  if(ranged.indexOf(type)>=0){
    // ★Bug Fix: 演算子優先度の罠 — `n<isCrit?18:10` は (n<isCrit)?18:10 となり常時truthyで無限ループ。
    // 括弧で囲んでループ上限として評価させる。
    var _rangedMax=isCrit?18:10;
    for(var n=0;n<_rangedMax;n++){
      var off=(n-(isCrit?9:5))*5;
      spawnProjectileArc(x1,y1+off,x2,y2+off*.35,col,7,180);
    }
    return;
  }
  for(var s=0;s<(isCrit?4:2);s++){
    var off2=(s-(isCrit?1.5:.5))*16;
    spawnMeleeSlash(x1,y1+off2,x2,y2-off2,col);
  }
  if(type==='assassin'||type==='ninja')spawnShockwave(x2,y2,'#9f6bff',isCrit?94:58);
}
...
function drawSprite(cvs,type,col,flip,isMaster){
  ...
  drawCinematicSpriteFinish(c,w,h,type,col,isMaster);
  // ★マスター装飾: 王冠 + 金色マント + 発光剣
  if(isMaster){
    var cx=w*.5;
    // 王冠
...
function drawCinematicSpriteFinish(c,w,h,type,col,isMaster){
  var cx=w*.5,def=UDEFS[type]||{},ei=ELEM_INFO[def.elem||'none']||ELEM_INFO.none,ec=ei.col||'#ffffff';
  var heavy=TYPE_CAT[type]==='heavy'||['king','paladin','knight','golem','titan','dragonknight'].indexOf(type)>=0;
  var magic=def.atkType==='magic'||TYPE_CAT[type]==='magic';
  var beast=['dragon','phoenix'].indexOf(type)>=0;
  c.save();
  // Cinematic rim light from the upper left and enemy-side red fill.
  c.globalCompositeOperation='screen';
  var rim=c.createLinearGradient(0,0,w,h);
  rim.addColorStop(0,'rgba(255,255,255,.34)');
  rim.addColorStop(.42,'rgba(160,210,255,.10)');
  rim.addColorStop(1,'rgba(255,80,40,.08)');
  c.fillStyle=rim;
  c.beginPath();c.ellipse(cx,h*.52,w*.36,h*.44,0,0,Math.PI*2);c.fill();
  var aura=c.createRadialGradient(cx,h*.52,w*.08,cx,h*.52,w*.55);
  aura.addColorStop(0,ec+'1f');
  aura.addColorStop(.58,ec+'12');
  aura.addColorStop(1,'rgba(0,0,0,0)');
  c.fillStyle=aura;c.fillRect(0,0,w,h);
  c.globalCompositeOperation='source-over';

  if(heavy){
    drawArmorAccents(c,w,h,col);
  }
  if(magic){
    drawArcaneAccents(c,w,h,ec,isMaster);
  }
  if(beast){
    drawBeastHighlights(c,w,h,ec);
  }
  if(['soldier','hero','dualblader','assassin','ninja','berserker','pirate','monk','spy'].indexOf(type)>=0){
    drawWeaponSheen(c,w,h,ec,type);
  }
  // Contact shadow and grounded dust make the unit feel less like a flat sticker.
  c.fillStyle='rgba(0,0,0,.28)';
  c.beginPath();c.ellipse(cx,h*.925,w*.28,h*.035,0,0,Math.PI*2);c.fill();
  c.restore();
}
function drawArmorAccents(c,w,h,col){
  var cx=w*.5,hi='rgba(240,250,255,.72)',mid='rgba(190,210,230,.32)',dk='rgba(0,0,0,.38)';
  c.save();
  c.lineCap='round';c.lineJoin='round';
  c.strokeStyle=hi;c.lineWidth=Math.max(1,w*.018);
  c.beginPath();c.moveTo(cx-w*.19,h*.41);c.quadraticCurveTo(cx,h*.35,cx+w*.19,h*.41);c.stroke();
  c.strokeStyle=mid;c.lineWidth=Math.max(1,w*.014);
  for(var i=0;i<3;i++){
    var y=h*(.47+i*.065);
    c.beginPath();c.moveTo(cx-w*.2,y);c.quadraticCurveTo(cx,y+h*.025,cx+w*.2,y);c.stroke();
  }
  c.fillStyle=dk;
  c.beginPath();c.ellipse(cx,h*.64,w*.19,h*.028,0,0,Math.PI*2);c.fill();
  c.fillStyle='rgba(255,255,255,.18)';
  c.beginPath();c.ellipse(cx-w*.11,h*.49,w*.045,h*.15,-.25,0,Math.PI*2);c.fill();
  c.restore();
}
function drawArcaneAccents(c,w,h,col,isMaster){
  var cx=w*.5;
  c.save();
  c.globalCompositeOperation='screen';
  c.strokeStyle=col;c.lineWidth=Math.max(1,w*.012);
  c.globalAlpha=isMaster ? .72 : .48;
  c.beginPath();c.arc(cx,h*.52,w*.36,-.35*Math.PI,1.05*Math.PI);c.stroke();
  c.beginPath();c.arc(cx,h*.52,w*.27,.15*Math.PI,1.65*Math.PI);c.stroke();
  for(var i=0;i<6;i++){
    var a=i/6*Math.PI*2+Date.now()/1300;
    var x=cx+Math.cos(a)*w*.34,y=h*.52+Math.sin(a)*h*.27;
    var g=c.createRadialGradient(x,y,0,x,y,w*.035);
    g.addColorStop(0,'#fff');g.addColorStop(.38,col);g.addColorStop(1,'transparent');
    c.fillStyle=g;c.beginPath();c.arc(x,y,w*.035,0,Math.PI*2);c.fill();
  }
  c.restore();
}
function drawBeastHighlights(c,w,h,col){
  var cx=w*.5;
  c.save();
  c.strokeStyle='rgba(255,255,255,.42)';c.lineWidth=Math.max(1,w*.012);c.lineCap='round';
  for(var i=0;i<4;i++){
    var y=h*(.34+i*.09);
    c.beginPath();c.moveTo(cx-w*.2,y);c.quadraticCurveTo(cx-w*.02,y-h*.03,cx+w*.18,y+h*.02);c.stroke();
  }
  c.globalCompositeOperation='screen';
  var glow=c.createRadialGradient(cx+w*.18,h*.3,0,cx+w*.18,h*.3,w*.22);
  glow.addColorStop(0,col+'55');glow.addColorStop(1,'transparent');
  c.fillStyle=glow;c.fillRect(0,0,w,h);
  c.restore();
}
function drawWeaponSheen(c,w,h,col,type){
  var cx=w*.5;
  c.save();
  c.globalCompositeOperation='screen';
  var x1=type==='ninja'||type==='assassin'?cx-w*.28:cx+w*.18;
  var y1=type==='ninja'||type==='assassin'?h*.72:h*.78;
  var x2=type==='ninja'||type==='assassin'?cx+w*.26:cx+w*.42;
  var y2=type==='ninja'||type==='assassin'?h*.28:h*.22;
  var g=c.createLinearGradient(x1,y1,x2,y2);
  g.addColorStop(0,'rgba(255,255,255,0)');
  g.addColorStop(.5,'rgba(255,255,255,.85)');
  g.addColorStop(1,col+'66');
  c.strokeStyle=g;c.lineWidth=Math.max(2,w*.018);c.lineCap='round';
...
function drawUnit(c,u,t){
  ...
  // ★ 未行動: 白文字（黒縁付き・視認性MAX）
  // ★ 行動済み: くすんだグレー文字
  var sym=UDEFS[u.type]?(typeof unitSym==='function'?unitSym(u.type,u):UDEFS[u.type].sym):'?';
  // ★マスター: 記号を金色に
