...
function showBattle(res,cb){
  ...
  if(atkMaster){
    ...
    if(lnEl){
      lnEl.innerHTML='<span style="color:#ffcc44">👑MASTER</span> '+lac.name+' '+UDEFS[res.atkType].name;
    }
  }
  if(defMaster){
    var rnEl=document.getElementById('bRName');
    if(rnEl){
      rnEl.innerHTML='<span style="color:#ffcc44">👑MASTER</span> '+rac.name+' '+UDEFS[res.defType].name;
    }
  }
